...
selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    at::Tensor last_state = torch::empty({batch_size, dim, dstate}, u.options().dtype(input_type));
...
selective_scan_bwd(const at::Tensor &u, const at::Tensor &delta,
    ...
    if (ssm_initial_state_.has_value()) { 
        dssm_initial_state = torch::empty({batch_size, dim, dstate}, u.options());
    ...
    at::Tensor last_state = torch::empty({batch_size, dim, dstate}, u.options());
